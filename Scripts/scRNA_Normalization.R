...
gene <- read.table("/datg/luping/WORKFLOW/Tanglab_Single_Cell_RNA_Seq/Tanglab_Single_Cell_RNA_Seq_cellranger-mm10-3.0.0/database/genes/gene_id_gene_name_gene_unique_name.txt",header=T,row.names=1)
barcode <- read.table("/datg/luping/WORKFLOW/Tanglab_Single_Cell_RNA_Seq/Tanglab_Single_Cell_RNA_Seq_cellranger-mm10-3.0.0/scripts/barcode_96_8bp.txt",row.names = 1)
...
rownames(UMI_count_final) <- as.character(gene$gene_unique_name)
